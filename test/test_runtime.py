...
def bench_against_spconv(
    ...
    with torch.no_grad():
        start_time = time()
        for i in range (loop):
            spconv_sparse = sp_conv(spconv_tensor)
            spconv_tensor.indice_dict.clear()
        end_time = time()
    ...
    with torch.no_grad():
        start_time = time()
        for i in range (loop):
            sphconv_sparse:spconv.SparseConvTensor = sph_conv(sphconv_tensor)
            sphconv_tensor.rule_cache.clear()
        end_time = time()
...
class TestClass:
    def test_speed(self):
        ...
        bench_against_spconv(
            loop=10, batch_size=4, in_channels=64, out_channels=128, spatial_shape_HWD=[128, 128, 20],
