...
template <typename scalar_t, int H_TILE, int W_TILE>
__global__ void sphconv_cuda_forward_kernel(
    const torch::GenericPackedTensorAccessor<scalar_t, 5, RestrictPtrTraits,
                                             size_t>
        feature,
    const torch::GenericPackedTensorAccessor<scalar_t, 4, RestrictPtrTraits,
                                             size_t>
        depth,
    const torch::GenericPackedTensorAccessor<scalar_t, 3, RestrictPtrTraits,
                                             size_t>
        thick,
    const torch::GenericPackedTensorAccessor<scalar_t, 5, RestrictPtrTraits,
                                             size_t>
    ...
    torch::GenericPackedTensorAccessor<scalar_t, 5, RestrictPtrTraits, size_t>
        new_feature,
    torch::GenericPackedTensorAccessor<scalar_t, 4, RestrictPtrTraits, size_t>
        new_depth,
    torch::GenericPackedTensorAccessor<scalar_t, 3, RestrictPtrTraits, size_t>
        new_thick,
...
sphconv_cuda_forward(torch::Tensor feature, torch::Tensor depth,
  ...
  AT_DISPATCH_FLOATING_TYPES(
      feature.type(), "sphconv_forward_cuda", ([&] {
        sphconv_cuda_forward_kernel<scalar_t, H_TILE,
                                    W_TILE><<<grid_size, block_size>>>(
            feature.generic_packed_accessor<scalar_t, 5,
                                            torch::RestrictPtrTraits, size_t>(),
            depth.generic_packed_accessor<int32_t, 4, torch::RestrictPtrTraits,
                                          size_t>(),
            ...
            bias.generic_packed_accessor<scalar_t, 1, torch::RestrictPtrTraits,
                                         size_t>(),
            new_feature.generic_packed_accessor<
                scalar_t, 5, torch::RestrictPtrTraits, size_t>(),
            new_depth.generic_packed_accessor<
                scalar_t, 4, torch::RestrictPtrTraits, size_t>(),
            new_thick.generic_packed_accessor<
                scalar_t, 3, torch::RestrictPtrTraits, size_t>(),
